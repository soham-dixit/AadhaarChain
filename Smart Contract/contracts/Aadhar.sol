...
contract Aadhar1 {
    ...
    function registerOperator(
        ...
        for (uint256 i = 1; i < nextOperatorId; i++) {
            if (
                keccak256(bytes(operators[i].email)) == keccak256(bytes(_email))
            ) {
                operatorExists = true;
                break;
            }
        ...
        if (!operatorExists) {
            operators[nextOperatorId] = Operator(
                nextOperatorId,
                _name,
                _email,
                _password,
    ...
    function loginOperator(
        ...
        for (uint256 i = 1; i < nextOperatorId; i++) {
            if (
                keccak256(bytes(operators[i].email)) ==
                keccak256(bytes(_email)) &&
                keccak256(bytes(operators[i].password)) ==
                keccak256(bytes(_password))
            ) {
    ...
    function addAppointment(
        ...
        appointments[_userId] = Appointment(
            _date,
            _time,
            _status,
            _appointmentType,
